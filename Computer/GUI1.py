...
class KUKAControlGUI:
    ...
    def start_motion(self):
        ...
        if not self.motion_active:
            try:
                # In a full implementation, you would send a motion start command
                # For now, we'll simulate this
                self.motion_active = True
                self.start_btn.config(state='disabled', text="MOTION ACTIVE")
                self.log_torque("MOTION STARTED - Rehabilitation drinking motion initiated")
                
                # Here you would send actual motion command to robot
                # Example: self.send_motion_command("START_DRINKING_MOTION")
    ...
    def apply_stiffness(self):
        ...
        try:
            ...
            self.log_torque(f"Stiffness updated: X={stiffness_x}, Y={stiffness_y}, Z={stiffness_z}, Rot={stiffness_rot}")
